...
//         enableFetchFromFollower = enableFetchFromFollower
//       )
//     }
//   }
  def createBrokerConfigs(
  ...
  def bootstrapServers[B <: KafkaBroker](
    ...
    brokers
      .map { s =>
        val listener = s.config.effectiveAdvertisedListeners
...
//         protocolAndPorts.map(p => "%s:%s".format(p._1, p._1)).mkString(",") + ",CONTROLLER:PLAINTEXT"
//       )
//     } else {
...
//     }
//     if (logDirCount > 1) {
//       val logDirs = (1 to logDirCount).toList
//         .map(i =>
...
//   }
  ...
  def createTopicWithAdminRaw[B <: KafkaBroker](
    ...
    val result = if (replicaAssignment.isEmpty) {
      admin.createTopics(
        Collections.singletonList(new NewTopic(topic, numPartitions, replicationFactor.toShort).configs(configsMap))
      )
    } else {
      ...
      admin.createTopics(Collections.singletonList(new NewTopic(topic, assignment).configs(configsMap)))
    }

    // FIXME HSTREAM: currently, hstream kafka doesn't support CreateTopic version 7+
    try {
      ...
      case e: ExecutionException
          if e.getCause != null &&
            // ignore
            e.getCause.isInstanceOf[UnsupportedVersionException] => {
        result.all().get()
        Uuid.ZERO_UUID
      }
    }
  ...
  def createTopicWithAdmin[B <: KafkaBroker](
      admin: Admin,
      topic: String,
      brokers: Seq[B], /* FIXME: Actually we don't need this for hstream, but we keep it for compatibility */
      numPartitions: Int = 1,
      replicationFactor: Int = 1,
      replicaAssignment: collection.Map[Int, Seq[Int]] = Map.empty,
      topicConfig: Properties = new Properties
    ...
    def isTopicExistsAndHasSameNumPartitionsAndReplicationFactor(cause: Throwable): Boolean = {
      ...
      !waitForAllPartitionsMetadata(admin, topic, effectiveNumPartitions).isEmpty &&
    ...
    val allPartitionsMetadata = waitForAllPartitionsMetadata(admin, topic, effectiveNumPartitions)

    (0 until effectiveNumPartitions).map { i =>
      i -> allPartitionsMetadata
        .get(new TopicPartition(topic, i))
        .map(_.leader())
        .getOrElse(
          throw new IllegalStateException(
            s"Cannot get the partition leader for topic: $topic, partition: $i in server metadata cache"
          )
        )
    }.toMap
  }
  ...
  def topicHasSameNumPartitionsAndReplicationFactor(
      adminClient: Admin,
      topic: String,
      numPartitions: Int,
  ...
  def deleteTopicWithAdmin[B <: KafkaBroker](
    ...
    waitForAllPartitionsMetadata(admin, topic, 0)
  }

  /**
   * Create a topic in ZooKeeper. Wait until the leader is elected and the metadata is propagated to all brokers. Return
   * the leader for each partition.
   */
  // There is also a createTopic in KafkaServerTestHarness
  //
...
//   }
   ...
   *   Security protocol which indicates if SASL or SSL or both configs are included
   * @param trustStoreFile
   *   Trust store file must be provided for SSL and SASL_SSL
   * @param certAlias
   *   Alias of certificate in SSL key store
   * @param certCn
   *   CN for certificate
   * @param saslProperties
   *   SASL configs if security protocol is SASL_SSL or SASL_PLAINTEXT
   * @param tlsProtocol
   *   TLS version
   * @param needsClientCert
   *   If not empty, a flag which indicates if client certificates are required. By default client certificates are
   *   generated only if securityProtocol is SSL (not for SASL_SSL).
   */
  def securityConfigs(
      mode: Mode,
      ...
      needsClientCert: Option[Boolean] = None
  ): Properties = {
    val props = new Properties
  ...
  def adminClientSecurityConfigs(
      securityProtocol: SecurityProtocol,
      trustStoreFile: Option[File],
      saslProperties: Option[Properties]
  ): Properties =
...
//   }
  ...
  /**
   * Execute the given block. If it throws an assert error, retry. Repeat until no error is thrown or the time limit
   * elapses
   */
  ...
  def subscribeAndWaitForRecords(
      topic: String,
   ...
   *
   * @param func
   *   The function defining the optional value
   * @param msg
   *   Error message in the case that the value never appears
   * @param waitTimeMs
   *   Maximum time to wait
   * @return
   *   The unwrapped value returned by the function
   */
   ...
   *   error message
   * @param waitTimeMs
   *   maximum time to wait and retest the condition before failing the test
   * @param pause
   ...
   */
  def tryUntilNoAssertionError[T](waitTime: Long = JTestUtils.DEFAULT_MAX_WAIT_MS, pause: Long = 100L)(
      assertions: => T
  ): T = {
...
//   }
   ...
   *
   * @param brokers
   *   The list of brokers that the metadata should reach to
   * @param topic
   *   The topic name
   * @param expectedNumPartitions
   *   The expected number of partitions
   * @return
   *   all partitions metadata
   */
  ...
  // }
  // For hstream
  def waitForAllPartitionsMetadata(
      admin: Admin,
      topic: String,
  ...
  /**
   * Wait until a valid leader is propagated to the metadata cache in each broker. It assumes that the leader propagated
   * to each broker is the same.
   *
   * @param brokers
   *   The list of brokers that the metadata should reach to
   * @param topic
   ...
   *   The partition Id
   * @param timeout
   *   The amount of time waiting on this condition before assert to fail
   * @return
  ...
  def waitForPartitionMetadata[B <: KafkaBroker](
      ...
      topic: String,
      partition: Int,
      timeout: Long = JTestUtils.DEFAULT_MAX_WAIT_MS
...
//   }
  ...
  def consumeTopicRecords[K, V, B <: KafkaBroker](
    ...
    val consumer = createConsumer(
      bootstrapServers(brokers, ListenerName.forSecurityProtocol(securityProtocol)),
      groupId = groupId,
      securityProtocol = securityProtocol,
  ...
  def pollUntilAtLeastNumRecords[K, V](
      consumer: Consumer[K, V],
      numRecords: Int,
      waitTimeMs: Long = JTestUtils.DEFAULT_MAX_WAIT_MS
   ...
   * remaining messages in the partitions the consumer is subscribed to, the duration should be set high enough so that
   * the consumer has enough time to poll everything. This would be based on the number of expected messages left in the
   * topic, and should not be too large (ie. more than a second) in our tests.
   *
   * @return
   *   All the records consumed by the consumer within the specified duration.
   */
  def consumeRecordsFor[K, V](
      consumer: KafkaConsumer[K, V],
      duration: Long = JTestUtils.DEFAULT_MAX_WAIT_MS
  ): Seq[ConsumerRecord[K, V]] = {
    val startTime = System.currentTimeMillis()
    val records = new ArrayBuffer[ConsumerRecord[K, V]]()
    waitUntilTrue(
...
//   }
  ...
  def seedTopicWithNumberedRecords[B <: KafkaBroker](topic: String, numRecords: Int, brokers: Seq[B]): Unit = {
    val props = new Properties()
    props.put(ProducerConfig.ENABLE_IDEMPOTENCE_CONFIG, "true")
    props.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, plaintextBootstrapServers(brokers))
...
//   }
  ...
  def stringifyTopicPartitions(partitions: Set[TopicPartition]): String = {
    Json.encodeAsString(
      Map(
        "partitions" ->
          partitions.map(tp => Map("topic" -> tp.topic, "partition" -> tp.partition).asJava).asJava
      ).asJava
    )
  }
...
// For hstream
  ...
  def topicExists(admin: Admin, topic: String): Boolean = {
    try {
