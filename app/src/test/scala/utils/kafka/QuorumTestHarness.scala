...
//   val faultHandler = faultHandlerFactory.faultHandler
  ...
  // TODO
  def metaStoreConnect: String = "zk://127.0.0.1:2181"
...
//   val ZkClientEventThreadSuffix = "-EventThread"
  ...
  /**
   * Verify that a previous test that doesn't use QuorumTestHarness hasn't left behind an unexpected thread. This
   * assumes that brokers, ZooKeeper clients, producers and consumers are not created in another @BeforeClass, which is
   * true for core tests where this harness is used.
   */
  @BeforeAll
  def setUpClass(): Unit = {
