...
import org.apache.kafka.common.requests.{FetchMetadata => JFetchMetadata, FetchRequest, FetchResponse}
...
/**
 * Subclasses of `BaseConsumerTest` exercise the consumer and fetch request/response. This class complements those
 * classes with tests that require lower-level access to the protocol.
 */
class FetchRequestTest extends BaseFetchRequestTest {

  @Test
  def testBrokerRespectsPartitionsOrderAndSizeLimits(): Unit = {
    // KAFKA_TO_HSTREAM: compatible with old apis
    val fetchVersion = findProperApiVersion(ApiKeys.FETCH)
    ...
    def createConsumerFetchRequest(
    ...
    val fetchRequest3 =
      createConsumerFetchRequest(shuffledTopicPartitions3, Map(partitionWithLargeMessage1 -> messagesPerPartition))
    val fetchResponse3 = sendFetchRequest(leaderId, fetchRequest3)
    evaluateResponse3(fetchResponse3)
    if (fetchVersion >= 12) {
      val fetchRequest3V12 =
    ...
    val fetchRequest4 =
      createConsumerFetchRequest(shuffledTopicPartitions4, Map(partitionWithLargeMessage2 -> messagesPerPartition))
    val fetchResponse4 = sendFetchRequest(leaderId, fetchRequest4)
    evaluateResponse4(fetchResponse4)
    if (fetchVersion >= 12) {
      val fetchRequest4V12 =
        createConsumerFetchRequest(
          shuffledTopicPartitions4,
  ...
  @Test
  @Disabled("TODO: ENABLE_FOR_HSTREAM, fetch to non-leader")
  def testFetchRequestToNonReplica(): Unit = {
    ...
    val partitionToLeader = createTopic(topic, numPartitions = 1, 1)
    val topicIds = getTopicIds().asJava
    val topicNames = topicIds.asScala.map(_.swap).asJava
    ...
    val fetchRequest = FetchRequest.Builder
      .forConsumer(ApiKeys.FETCH.latestVersion, Int.MaxValue, 0, createPartitionMap(1024, Seq(topicPartition)))
      .build()
    ...
    val oldFetchRequest =
      FetchRequest.Builder.forConsumer(12, Int.MaxValue, 0, createPartitionMap(1024, Seq(topicPartition))).build()
...
//   }
  ...
  /**
   * Tests that down-conversions don't leak memory. Large down conversions are triggered in the server. The client
   * closes its connection after reading partial data when the channel is muted in the server. If buffers are not
   * released this will result in OOM.
   */
  @Test
  @Disabled("TODO: ENABLE_FOR_HSTREAM DownConversion, also see testHStreamNoDownConversionWithConnectionFailure")
  // Disabled reason details: this test send a fetchResponse v1 to the broker, which expect
  // a legacy MessageSet response. The condition `assertEquals(3, batches.size)`
  // is true for the legacy MessageSet response, but not for the new RecordBatch.
  //
  // And hstream don't support legacy MessageSet response, so this test is disabled.
  ...
  @Test
  // Default junit timeout is 60s and this test can take longer than that in the ci environment
  @Timeout(value = 300, unit = TimeUnit.SECONDS, threadMode = Timeout.ThreadMode.SEPARATE_THREAD)
  def testHStreamNoDownConversionWithConnectionFailure(): Unit = {
    val (topicPartition, leaderId) = createTopics(numTopics = 1, numPartitions = 1).head
    val topicIds = getTopicIds().asJava
    val topicNames = topicIds.asScala.map(_.swap).asJava
    ...
    val version = 4.toShort
    (0 to 15).foreach(_ => fetch(version, maxPartitionBytes = msgValueLen * 1000, closeAfterPartialResponse = true))

    val response1 = fetch(version, maxPartitionBytes = 4 * msgValueLen, closeAfterPartialResponse = false)
    val fetchResponse1 = response1.getOrElse(throw new IllegalStateException("No fetch response"))
    val partitionData1 = fetchResponse1.responseData(topicNames, version).get(topicPartition)
    assertEquals(Errors.NONE.code, partitionData1.errorCode)
    val batches1 = FetchResponse.recordsOrFail(partitionData1).batches.asScala.toBuffer
    assertEquals(1, batches1.size)

    val response2 = fetch(version, maxPartitionBytes = 7 * msgValueLen, closeAfterPartialResponse = false)
    val fetchResponse2 = response2.getOrElse(throw new IllegalStateException("No fetch response"))
    val partitionData2 = fetchResponse2.responseData(topicNames, version).get(topicPartition)
    assertEquals(Errors.NONE.code, partitionData2.errorCode)
    val batches2 = FetchResponse.recordsOrFail(partitionData2).batches.asScala.toBuffer
    assertEquals(2, batches2.size)
  }

  /**
   * Ensure that we respect the fetch offset when returning records that were converted from an uncompressed v2 record
   * batch to multiple v0/v1 record batches with size 1. If the fetch offset points to inside the record batch, some
   * records have to be dropped during the conversion.
   */
  @Test
  @Disabled(
    "TODO: ENABLE_FOR_HSTREAM DownConversion, also see testHStreamNoDownConversionFromBatchedToUnbatchedRespectsOffset"
  )
  ...
  @Test
  def testHStreamNoDownConversionFromBatchedToUnbatchedRespectsOffset(): Unit = {
    // Increase linger so that we have control over the batches created
    producer = TestUtils.createProducer(
      bootstrapServers(),
      retries = 5,
...
//   }
  ...
  @Test
  @Disabled("TODO: ENABLE_FOR_HSTREAM BrokerCompressionType")
  def testZStdCompressedTopic(): Unit = {
    ...
    producer.send(new ProducerRecord(topicPartition.topic, topicPartition.partition, "key1", "value1")).get
    producer.send(new ProducerRecord(topicPartition.topic, topicPartition.partition, "key2", "value2")).get
    producer.send(new ProducerRecord(topicPartition.topic, topicPartition.partition, "key3", "value3")).get
    producer.close()

    // fetch request with version below v10: UNSUPPORTED_COMPRESSION_TYPE error occurs
    val req0 =
      new FetchRequest.Builder(0, 9, -1, -1, Int.MaxValue, 0, createPartitionMap(300, Seq(topicPartition), Map.empty))
        .setMaxBytes(800)
        .build()
    ...
    val req1 =
      new FetchRequest.Builder(0, 10, -1, -1, Int.MaxValue, 0, createPartitionMap(300, Seq(topicPartition), Map.empty))
        .setMaxBytes(800)
        .build()
  ...
  @Test
  @Disabled("TODO: ENABLE_FOR_HSTREAM, see testHStreamZStdCompressedRecords")
  // HStream: actually, we don't support down-conversion(magic 0 and 1), so this test is disabled.
  // Also version 10 is not supported in HStream currently.
  def testZStdCompressedRecords(): Unit = {
    ...
    producer1.send(new ProducerRecord(topicPartition.topic, topicPartition.partition, "key1", "value1")).get
    producer1.close()
    // Produce ZSTD compressed messages (v2)
    val producer2 = TestUtils.createProducer(
      bootstrapServers(),
      compressionType = CompressionType.ZSTD.name,
      keySerializer = new StringSerializer,
      valueSerializer = new StringSerializer
    )
    producer2.send(new ProducerRecord(topicPartition.topic, topicPartition.partition, "key2", "value2")).get
    producer2.send(new ProducerRecord(topicPartition.topic, topicPartition.partition, "key3", "value3")).get
    producer2.close()
    ...
    val req0 =
      new FetchRequest.Builder(0, 1, -1, -1, Int.MaxValue, 0, createPartitionMap(300, Seq(topicPartition), Map.empty))
        .setMaxBytes(800)
        .build()
    ...
    val req1 = new FetchRequest.Builder(
      0,
      1,
      -1,
      -1,
    ...
    // zstd compressed record raises UNSUPPORTED_COMPRESSION_TYPE error.
    val req2 =
      new FetchRequest.Builder(2, 3, -1, -1, Int.MaxValue, 0, createPartitionMap(300, Seq(topicPartition), Map.empty))
        .setMaxBytes(800)
        .build()
    ...
    val req3 = new FetchRequest.Builder(
      0,
      1,
      -1,
      -1,
    ...
    val req5 = new FetchRequest.Builder(
      0,
      ApiKeys.FETCH.latestVersion(),
      -1,
      -1,
  ...
  @Test
  def testHStreamZStdCompressedRecords(): Unit = {
    // Producer compressed topic
    val topicConfig = Map(TopicConfig.COMPRESSION_TYPE_CONFIG -> BrokerCompressionType.PRODUCER.name)
    val (topicPartition, leaderId) = createTopics(numTopics = 1, numPartitions = 1, configs = topicConfig).head
    val topicIds = getTopicIds().asJava
    ...
    val fetchVersion = findProperApiVersion(ApiKeys.FETCH)
    // fetch request with fetch version v4+ (magic 2): works fine
    val req1 =
    ...
    val res1 = sendFetchRequest(leaderId, req1)
    val data1 = res1.responseData(topicNames, fetchVersion).get(topicPartition)
    assertEquals(Errors.NONE.code, data1.errorCode)
    assertEquals(3, records(data1).size)
    ...
    val res2 = sendFetchRequest(leaderId, req2)
    val data2 = res2.responseData(topicNames, fetchVersion).get(topicPartition)
    assertEquals(Errors.NONE.code, data2.errorCode)
    assertEquals(2, records(data2).size)
  }

  private def checkFetchResponse(
      expectedPartitions: Seq[TopicPartition],
      fetchResponse: FetchResponse,
      maxPartitionBytes: Int,
      maxResponseBytes: Int,
      numMessagesPerPartition: Int,
      // KAFKA_TO_HSTREAM
      // responseVersion: Short = ApiKeys.FETCH.latestVersion()
      responseVersion: Short = findProperApiVersion(ApiKeys.FETCH)
  ): Unit = {
  ...
  def testHStreamLargeMinBytes(): Unit = {
    initProducer()
    val fetchVersion = findProperApiVersion(ApiKeys.FETCH)
    ...
    // Each record is about 78 bytes, we wrote 2 records. maxResponseBytes
    // should be large enough to fetch one record.
    val maxResponseBytes = 100
    // Broker will Fetch with fetchMinBytes which is Math.min(fetchRequest.minBytes, fetchMaxBytes)
    val minBytes = Int.MaxValue

    val req = createConsumerFetchRequest(
      maxResponseBytes,
      400, /* maxPartitionBytes */
      Seq(topicPartition),
      Map.empty,
      fetchVersion,
      maxWaitMs = Int.MaxValue,
      minBytes = minBytes
    )
    val res = sendFetchRequest(leaderId, req)
    val data = res.responseData(topicNames, fetchVersion).get(topicPartition)
