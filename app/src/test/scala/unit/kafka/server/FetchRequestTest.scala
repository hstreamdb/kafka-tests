...
class FetchRequestTest extends BaseFetchRequestTest {
  ...
  @Test
  @Disabled("test failed with unexpected partition assignment")
  ...
  @Test
  @Disabled("need support transaction")
  ...
  @Test
  @Disabled("Server should currectly handle NOT_LEADER_OR_FOLLOWER error")
...
//
   ...
   */
  @Test
  @Disabled("Reasons for failure need to be investigated in depth")
      ...
      try {
        ...
        if (closeAfterPartialResponse) {
          ...
          assertTrue(size > maxPartitionBytes - batchSize,
            s"Fetch size too small $size, broker may have run out of memory. maxPartitionBytes: $maxPartitionBytes, batchSize: $batchSize")
          None
        } else {
