...
//     properties.put(KafkaConfig.TransactionsTopicMinISRProp, "1")
  ...
  override def setUp(testInfo: TestInfo): Unit = {
    ...
    // ENABLE_FOR_HSTREAM: currently HStream doesn't support creating offsets topic
  ...
  def testOffsetFetchRequestSingleGroup(): Unit = {
    ...
    for (version <- 1 to 3) {
      if (version < 8) {
        val request =
          if (version < 7) {
            new OffsetFetchRequest.Builder(groupId, false, tpList, false)
              .build(version.asInstanceOf[Short])
          } else {
            new OffsetFetchRequest.Builder(groupId, false, tpList, true)
              .build(version.asInstanceOf[Short])
          }
        ...
        verifySingleGroupResponse(
          version.asInstanceOf[Short],
          response.error().code(),
          partitionData.errorCode(),
          topicData.name(),
          partitionData.partitionIndex(),
          partitionData.committedOffset(),
          partitionData.committedLeaderEpoch(),
          partitionData.metadata()
        )
      } else {
        val request = new OffsetFetchRequest.Builder(Map(groupId -> tpList).asJava, false, false)
          .build(version.asInstanceOf[Short])
        val response = connectAndReceive[OffsetFetchResponse](request)
  ...
  @Test
  @Disabled("TODO: ENABLE_FOR_HSTREAM, need to support OffsetFetchRequest >= V8")
  ...
  private def verifySingleGroupResponse(
    ...
    // TODO: ENABLE_FOR_HSTREAM, currently metadata is not stored when commit
  ...
  private def verifyResponse(
      groupLevelResponse: Errors,
      partitionData: util.Map[TopicPartition, PartitionData],
      topicList: util.List[TopicPartition]
  ): Unit = {
  ...
  private def commitOffsets(tpList: util.List[TopicPartition]): Unit = {
    ...
    val offsets = tpList.asScala
      .map { tp =>
        (tp, new OffsetAndMetadata(offset, leaderEpoch, metadata))
      }
      .toMap
      .asJava
