...
abstract class AbstractCreateTopicsRequestTest extends BaseRequestTest {
  ...
  def topicsReq(topics: Seq[CreatableTopic], timeout: Integer = 10000, validateOnly: Boolean = false) = {
    ...
    new CreateTopicsRequest.Builder(req).build(findProperApiVersion(ApiKeys.CREATE_TOPICS))
  }
    ...
    request.data.topics.forEach { topic =>
      def verifyMetadata(socketServer: SocketServer) = {
        val metadata = sendMetadataRequest(
          new MetadataRequest.Builder(List(topic.name()).asJava, false).build(findProperApiVersion(ApiKeys.METADATA)),
          socketServer
        ).topicMetadata.asScala
      ...
      verifyMetadata(anySocketServer)
    }
  }
  ...
  protected def validateErrorCreateTopicsRequests(
  ...
  protected def validateTopicExists(topic: String): Unit = {
    ...
    val metadata = sendMetadataRequest(
      new MetadataRequest.Builder(List(topic).asJava, true).build(findProperApiVersion(ApiKeys.METADATA))
    ).topicMetadata.asScala
