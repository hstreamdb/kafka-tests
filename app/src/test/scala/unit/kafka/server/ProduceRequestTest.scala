...
/**
 * Subclasses of `BaseProduceSendRequestTest` exercise the producer and produce request/response. This class complements
 * those classes with tests that require lower-level access to the protocol.
 */
class ProduceRequestTest extends BaseRequestTest {
    ...
    def sendAndCheck(memoryRecords: MemoryRecords, expectedOffset: Long): Unit = {
      ...
      val produceResponse = sendProduceRequest(
        ...
        ProduceRequest
          ...
          .build(findProperApiVersion(ApiKeys.PRODUCE))
      )
  ...
  def testProduceWithInvalidTimestamp(): Unit = {
    ...
    val partitionToLeader = createTopic(topic, 1, 1, topicConfig)
  ...
  def testZSTDProduceRequest(): Unit = {
    ...
    // KAFKA_ORIGINAL
    // val partitonToLeader = TestUtils.createTopic(zkClient, topic, 1, 1, servers, topicConfig)
    val partitionToLeader = createTopic(topic, 1, 1, topicConfig)
    val leader = partitionToLeader(partition)
