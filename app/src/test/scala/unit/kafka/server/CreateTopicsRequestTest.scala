...
class CreateTopicsRequestTest extends AbstractCreateTopicsRequestTest {
  ...
  def testValidCreateTopicsRequests( /*quorum: String*/ ): Unit = {
    ...
    validateValidCreateTopicsRequests(
      topicsReq(
        Seq(topicReq("topic3", numPartitions = 5, replicationFactor = 2, config = Map("min.insync.replicas" -> "2")))
      )
    )
    // TODO ENABLE_FOR_HSTREAM: topic assignments
    // // Manual assignments
    ...
    //   )
    // )
    // TODO ENABLE_FOR_HSTREAM: CreateTopicRequest version 4+
    // // Defaults
    // validateValidCreateTopicsRequests(topicsReq(Seq(topicReq("topic12", replicationFactor = -1, numPartitions = -1))))
    // validateValidCreateTopicsRequests(topicsReq(Seq(topicReq("topic13", replicationFactor = 2, numPartitions = -1))))
    // validateValidCreateTopicsRequests(topicsReq(Seq(topicReq("topic14", replicationFactor = -1, numPartitions = 2))))
  }
  ...
  def testErrorCreateTopicsRequests( /*quorum: String*/ ): Unit = {
    ...
    validateErrorCreateTopicsRequests(
      topicsReq(Seq(topicReq(existingTopic))),
      Map(existingTopic -> error(Errors.TOPIC_ALREADY_EXISTS, Some("Topic 'existing-topic' already exists.")))
    )
    validateErrorCreateTopicsRequests(
      topicsReq(Seq(topicReq("error-partitions", numPartitions = -2))),
      Map("error-partitions" -> error(Errors.INVALID_PARTITIONS)),
      checkErrorMessage = false
    )
    // TODO ENABLE_FOR_HSTREAM
    // validateErrorCreateTopicsRequests(
    //   topicsReq(Seq(topicReq("error-replication", replicationFactor = brokerCount + 1))),
    //   Map("error-replication" -> error(Errors.INVALID_REPLICATION_FACTOR)),
    //   checkErrorMessage = false
    // )
    // TODO ENABLE_FOR_HSTREAM
    // validateErrorCreateTopicsRequests(
    //   topicsReq(Seq(topicReq("error-config", config = Map("not.a.property" -> "error")))),
    //   Map("error-config" -> error(Errors.INVALID_CONFIG)),
    //   checkErrorMessage = false
    // )
    // validateErrorCreateTopicsRequests(
    //   topicsReq(Seq(topicReq("error-config-value", config = Map("message.format.version" -> "invalid-value")))),
    //   Map("error-config-value" -> error(Errors.INVALID_CONFIG)),
    //   checkErrorMessage = false
    // )
    // TODO ENABLE_FOR_HSTREAM
    // validateErrorCreateTopicsRequests(
    //   topicsReq(Seq(topicReq("error-assignment", assignment = Map(0 -> List(0, 1), 1 -> List(0))))),
  ...
  def testCreateTopicsWithVeryShortTimeouts( /*quorum: String*/ ): Unit = {
    ...
    validateErrorCreateTopicsRequests(
      topicsReq(Seq(topicReq("error-timeout", numPartitions = 10, replicationFactor = 3)), timeout = 1),
      Map("error-timeout" -> error(Errors.REQUEST_TIMED_OUT)),
      checkErrorMessage = false
    )
    validateErrorCreateTopicsRequests(
      topicsReq(Seq(topicReq("error-timeout-zero", numPartitions = 10, replicationFactor = 3)), timeout = 0),
      Map("error-timeout-zero" -> error(Errors.REQUEST_TIMED_OUT)),
      checkErrorMessage = false
    )
    // Negative timeouts are treated the same as 0
    validateErrorCreateTopicsRequests(
      topicsReq(Seq(topicReq("error-timeout-negative", numPartitions = 10, replicationFactor = 3)), timeout = -1),
      Map("error-timeout-negative" -> error(Errors.REQUEST_TIMED_OUT)),
      checkErrorMessage = false
    )
  ...
  def testInvalidCreateTopicsRequests( /*quorum: String*/ ): Unit = {
    ...
    validateErrorCreateTopicsRequests(
      topicsReq(
        Seq(topicReq("bad-args-topic", numPartitions = 10, replicationFactor = 3, assignment = Map(0 -> List(0))))
      ),
...
//   }
  ...
  def testCreateTopicsRequestVersions( /*quorum: String*/ ): Unit = {
    ...
    for (version <- findMinApiVersion(ApiKeys.CREATE_TOPICS) to findProperApiVersion(ApiKeys.CREATE_TOPICS)) {
      ...
      data.setTopics(
        new CreatableTopicCollection(
          List(
            topicReq(topic, numPartitions = 1, replicationFactor = 1, config = Map("min.insync.replicas" -> "2"))
          ).asJava.iterator()
        )
      )
