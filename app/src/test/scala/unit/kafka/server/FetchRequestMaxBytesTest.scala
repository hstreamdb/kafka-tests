...
class FetchRequestMaxBytesTest extends BaseRequestTest {
  ...
  private val messages =
    IndexedSeq(multiByteArray(1), multiByteArray(500), multiByteArray(1040), multiByteArray(500), multiByteArray(50))
  ...
  private def createTopics(): Unit = {
    ...
    createTopic(testTopic, numPartitions = 1, replicationFactor = 1, topicConfig = topicConfig)
   ...
   *
   * Note that when a single batch is larger than FetchMaxBytes, it will be returned in full even if this is larger than
   * FetchMaxBytes. See KIP-74.
   */
  @Test
  ...
  private def expectNextRecords(expected: IndexedSeq[Array[Byte]], fetchOffset: Long): Unit = {
    val response = sendFetchRequest(
      0,
      FetchRequest.Builder
        .forConsumer(
          ...
          Map(
            testTopicPartition ->
              new PartitionData(Uuid.ZERO_UUID, fetchOffset, 0, Integer.MAX_VALUE, Optional.empty())
          ).asJava
        )
        .build(3)
    )
    val records =
      FetchResponse.recordsOrFail(response.responseData(getTopicNames().asJava, 3).get(testTopicPartition)).records()
    ...
    recordsList.zipWithIndex.foreach {
      case (record, i) => {
        ...
        buffer.get(array)
        assertArrayEquals(expected(i), array, s"expectNextRecords unexpected element ${i}")
      }
    }
  }
}
