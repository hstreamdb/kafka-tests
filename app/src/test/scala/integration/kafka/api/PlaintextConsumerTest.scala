...
class PlaintextConsumerTest extends BaseConsumerTest {
  ...
  private def testHeadersSerializeDeserialize(
      serializer: Serializer[Array[Byte]],
      deserializer: Deserializer[Array[Byte]]
  ): Unit = {
    val numRecords = 1
    val record = new ProducerRecord(tp.topic, tp.partition, null, "key".getBytes, "value".getBytes)

    val producer = createProducer(keySerializer = new ByteArraySerializer, valueSerializer = serializer)
    producer.send(record)

    val consumer = createConsumer(keyDeserializer = new ByteArrayDeserializer, valueDeserializer = deserializer)
    assertEquals(0, consumer.assignment.size)
    consumer.assign(List(tp).asJava)
    assertEquals(1, consumer.assignment.size)
  ...
  /**
   * Verifies that pattern subscription performs as expected. The pattern matches the topics 'topic' and 'tblablac', but
   * not 'tblablak' or 'tblab1'. It is expected that the consumer is subscribed to all partitions of 'topic' and
   * 'tblablac' after the subscription when metadata is refreshed. When a new topic 'tsomec' is added afterwards, it is
   * expected that upon the next metadata refresh the consumer becomes subscribed to this new topic and all partitions
  ...
  def testPatternSubscription(): Unit = {
    ...
    assignment ++= Set(new TopicPartition(topic4, 0), new TopicPartition(topic4, 1))
    awaitAssignment(consumer, assignment)
  ...
  /**
   * Verifies that a second call to pattern subscription succeeds and performs as expected. The initial subscription is
   * to a pattern that matches two topics 'topic' and 'foo'. The second subscription is to a pattern that matches 'foo'
   * and a new topic 'bar'. It is expected that the consumer is subscribed to all partitions of 'topic' and 'foo' after
   * the first subscription, and to all partitions of 'foo' and 'bar' after the second. The metadata refresh interval is
   * intentionally increased to a large enough value to guarantee that it is the subscription call that triggers a
   * metadata refresh, and not the timeout.
   */
  @Test
  def testSubsequentPatternSubscription(): Unit = {
    ...
    var assignment = Set(new TopicPartition(topic, 0), new TopicPartition(topic, 1), new TopicPartition(fooTopic, 0))
    awaitAssignment(consumer, assignment)
    ...
    assignment --= Set(new TopicPartition(topic, 0), new TopicPartition(topic, 1))
    assignment ++= Set(new TopicPartition(barTopic, 0))
    awaitAssignment(consumer, assignment)
  ...
  /**
   * Verifies that pattern unsubscription performs as expected. The pattern matches the topics 'topic' and 'tblablac'.
   * It is expected that the consumer is subscribed to all partitions of 'topic' and 'tblablac' after the subscription
   * when metadata is refreshed. When consumer unsubscribes from all its subscriptions, it is expected that its
  ...
  def testExpandingTopicSubscriptions(): Unit = {
    ...
    val expandedAssignment =
      initialAssignment ++ Set(new TopicPartition(otherTopic, 0), new TopicPartition(otherTopic, 1))
  ...
  def testPositionAndCommit(): Unit = {
    ...
    assertEquals(
      ...
      "position() on a partition that we are subscribed to should reset the offset"
    )
    consumer.commitSync()
  ...
  def testMultiConsumerRoundRobinAssignor(): Unit = {
    ...
    val (consumerGroup, consumerPollers) =
      createConsumerGroupAndWaitForAssignment(10, List(topic1, topic2), subscriptions)
    try {
  ...
  /**
   * This test runs the following scenario to verify sticky assignor behavior. Topics: single-topic, with random number
   * of partitions, where #par is 10, 20, 30, 40, 50, 60, 70, 80, 90, or 100 Consumers: 9 consumers subscribed to the
   * single topic Expected initial assignment: partitions are assigned to consumers in a round robin fashion.
   *   - (#par mod 9) consumers will get (#par / 9 + 1) partitions, and the rest get (#par / 9) partitions
   * Then consumer #10 is added to the list (subscribing to the same single topic) Expected new assignment:
  ...
  /**
   * This test re-uses BaseConsumerTest's consumers. As a result, it is testing the default assignment strategy set by
   * BaseConsumerTest
   */
  @Test
  @Disabled("fix join group")
  ...
  def testRebalanceAndRejoin(assignmentStrategy: String): Unit = {
    ...
    TestUtils.waitUntilTrue(
      () => consumerPoller1.consumerAssignment() == expectedAssignment,
    ...
    TestUtils.waitUntilTrue(
      () => consumerPoller1.consumerAssignment().size == 1,
      s"Timed out while awaiting expected assignment size change to 1."
    )
    TestUtils.waitUntilTrue(
      () => consumerPoller2.consumerAssignment().size == 1,
      s"Timed out while awaiting expected assignment size change to 1."
    )
  ...
  def testInterceptors(): Unit = {
    ...
    val testConsumer =
      createConsumer(keyDeserializer = new StringDeserializer, valueDeserializer = new StringDeserializer)
  ...
  def testPerPartitionLagMetricsCleanUpWithAssign(): Unit = {
    ...
    awaitNonEmptyRecords(consumer, tp2)
    assertNull(
      consumer.metrics.get(new MetricName(tp.toString + ".records-lag", "consumer-fetch-manager-metrics", "", tags))
    )
  ...
  def testPerPartitionLagWithMaxPollRecords(): Unit = {
    ...
    assertEquals(
      ...
      lag.metricValue.asInstanceOf[Double],
      epsilon,
      s"The lag should be ${numMessages - records.count}"
    )
  }
...
//   }
   ...
   *   topics to which consumers will subscribe to
   * @return
   *   collection of consumer pollers
   */
  def subscribeConsumers(
   ...
   * @return
   *   collection of created consumers and collection of corresponding consumer pollers
   */
  def createConsumerGroupAndWaitForAssignment(
      consumerCount: Int,
      topicsToSubscribe: List[String],
      subscriptions: Set[TopicPartition]
  ): (Buffer[Consumer[Array[Byte], Array[Byte]]], Buffer[ConsumerAssignmentPoller]) = {
  ...
  def changeConsumerSubscriptionAndValidateAssignment[K, V](
  ...
  private def awaitNonEmptyRecords[K, V](consumer: Consumer[K, V], partition: TopicPartition): ConsumerRecords[K, V] = {
    TestUtils.pollRecordsUntilTrue(
      consumer,
      (polledRecords: ConsumerRecords[K, V]) => {
  ...
  def testConsumingWithNullGroupId(): Unit = {
    ...
    TestUtils.waitUntilTrue(
      () => {
        ...
        TestUtils.topicExists(createAdminClient(), topic)
      },
