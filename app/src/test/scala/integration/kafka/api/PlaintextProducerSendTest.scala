...
class PlaintextProducerSendTest extends BaseProducerSendTest {
  ...
  def testBatchSizeZero( /*quorum: String*/ ): Unit = {
    val producer = createProducer(lingerMs = Int.MaxValue, deliveryTimeoutMs = Int.MaxValue, batchSize = 0)
  ...
  def testBatchSizeZeroNoPartitionNoRecordKey( /*quorum: String*/ ): Unit = {
    ...
    try {
      ...
      val futures = for (i <- 1 to numRecords) yield {
        val record =
          new ProducerRecord[Array[Byte], Array[Byte]](topic, null, s"value$i".getBytes(StandardCharsets.UTF_8))
  ...
  def testSendWithInvalidCreateTime( /*quorum: String*/ ): Unit = {
    ...
    try {
      val e = assertThrows(
        ...
        () =>
          producer
            .send(new ProducerRecord(topic, 0, System.currentTimeMillis() - 1001, "key".getBytes, "value".getBytes))
            .get()
      ).getCause
  ...
  def testNonBlockingProducer( /*quorum: String*/ ): Unit = {
    ...
    def verifyMetadataNotAvailable(future: Future[RecordMetadata]): Unit = {
      ...
      assertEquals(
        ...
        assertThrows(classOf[ExecutionException], () => future.get).getCause.getClass
      )
    }
    ...
    def verifyBufferExhausted(future: Future[RecordMetadata]): Unit = {
      assertTrue(future.isDone) // verify future was completed immediately
      assertEquals(
        classOf[BufferExhaustedException],
        assertThrows(classOf[ExecutionException], () => future.get).getCause.getClass
      )
    }
    ...
    val producer2 = createProducer(maxBlockMs = 0, lingerMs = 15000, batchSize = 1100, bufferSize = 1500)
  ...
  def testSendRecordBatchWithMaxRequestSizeAndHigher( /*quorum: String*/ ): Unit = {
    ...
    val DEFAULT_MAX_MESSAGE_BYTES = 1024 * 1024 + Records.LOG_OVERHEAD
    val valueSize = DEFAULT_MAX_MESSAGE_BYTES - overhead
    ...
    assertEquals(
      classOf[RecordTooLargeException],
      assertThrows(classOf[ExecutionException], () => producer.send(record1).get).getCause.getClass
    )
  }
