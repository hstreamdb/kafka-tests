...
class ProducerFailureHandlingTest extends KafkaServerTestHarness {
  ...
  def generateConfigs =
    TestUtils.createBrokerConfigs(numServers, metaStoreConnect, testInfo).map(KafkaConfig.fromProps(_, overridingProps))
  ...
  override def setUp(testInfo: TestInfo): Unit = {
    ...
    producer1 = TestUtils.createProducer(
      bootstrapServers(),
      acks = 0,
      retries = 0,
    ...
    producer2 = TestUtils.createProducer(
      bootstrapServers(),
      acks = 1,
      retries = 0,
      requestTimeoutMs = 30000,
    ...
    producer3 = TestUtils.createProducer(
      bootstrapServers(),
      acks = -1,
      retries = 0,
      requestTimeoutMs = 30000,
   ...
   */
  @Test
  @Disabled("TODO ENABLE_FOR_HSTREAM: support ack=0 (although this test can be passed)")
   ...
   */
  @Disabled("TODO ENABLE_FOR_HSTREAM: support ack=1")
  @Test
  ...
  @Disabled("TODO ENABLE_FOR_HSTREAM: support set serverMessageMaxBytes")
  @Test
  def testHStreamTooLargeRecordWithAckMinusOne(): Unit = {
    // create topic
    createTopic(topic1, replicationFactor = numServers)

    // send a too-large record
    val record = new ProducerRecord(topic1, null, "key".getBytes, new Array[Byte](serverMessageMaxBytes + 1))
    assertThrows(classOf[ExecutionException], () => producer3.send(record).get)
  }
  ...
  private def checkTooLargeRecordForReplicationWithAckAll(maxFetchSize: Int): Unit = {
    ...
    val value =
      new Array[Byte](maxMessageSize - DefaultRecordBatch.RECORD_BATCH_OVERHEAD - DefaultRecord.MAX_RECORD_OVERHEAD)
   ...
   *
   * TODO: other exceptions that can be thrown in ExecutionException: UnknownTopicOrPartitionException
   * NotLeaderOrFollowerException LeaderNotAvailableException CorruptRecordException TimeoutException
   */
  @Test
  def testWrongBrokerList(): Unit = {
    ...
    producer4 = TestUtils.createProducer(
  ...
  /**
   * Send with invalid partition id should return ExecutionException caused by TimeoutException when partition is higher
   * than the upper bound of partitions.
   */
  @Test
  ...
  @Disabled("TODO ENABLE_FOR_HSTREAM: validate replicas")
  @Test
  ...
  def testNotEnoughReplicasAfterBrokerShutdown(): Unit = {
    ...
    val e = assertThrows(classOf[ExecutionException], () => producer3.send(record).get)
    assertTrue(
      e.getCause.isInstanceOf[NotEnoughReplicasException] ||
