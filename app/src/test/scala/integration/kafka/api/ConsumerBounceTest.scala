 ...
 import kafka.utils.{FixedPortTestUtils, Logging, TestUtils}
 ...
 class ConsumerBounceTest extends AbstractConsumerTest with Logging {
   ...
   this.consumerConfig.setProperty(ConsumerConfig.ALLOW_AUTO_CREATE_TOPICS_CONFIG, "false")
...
//     properties.put(KafkaConfig.UncleanLeaderElectionEnableProp, "true")
     ...
     FixedPortTestUtils.createBrokerConfigs(brokerCount, metaStoreConnect, testInfo)
   ...
   def consumeWithBrokerFailures(numIters: Int): Unit = {
     ...
     Thread.sleep(2000) // sleep sometime to make sure consumer group is active

     // 通过 docker ps -a 打印所有容器
     val cmd = Seq("docker", "ps", "-a")
...
//     ), "Failed to update high watermark for followers after timeout")
   ...
   @Test
   @Disabled("close broker auto create topic")
...
//     Thread.sleep(2000)
   ...
   @Test
   @Disabled("support FindCoordinator Request V4")
   def testClose(): Unit = {
     val numRecords = 10
     val producer = createProducer()
     producerSend(producer, numRecords)
...
//   }
   ...
   /**
     ...
     */
   @Test
   @Disabled("support set GroupMaxSizeProp for broker")
