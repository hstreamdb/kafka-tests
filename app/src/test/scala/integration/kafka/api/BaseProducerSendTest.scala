...
abstract class BaseProducerSendTest extends KafkaServerTestHarness {
  ...
  def generateConfigs: scala.collection.Seq[KafkaConfig] = {
    ...
    val numServers = sys.env.getOrElse("BROKER_COUNT", "2").toInt
  ...
  override def setUp(testInfo: TestInfo): Unit = {
    ...
    admin = TestUtils.createAdminClient(
      brokers,
      listenerName,
   ...
   *
   *   1. Send with null key/value/partition-id should be accepted; send with null topic should be rejected. 2. Last
   *      message of the non-blocking send should return the correct offset metadata
   */
  //
  ...
  def testSendOffset( /*quorum: String*/ ): Unit = {
    ...
    try {
      ...
      val record0 = new ProducerRecord[Array[Byte], Array[Byte]](
        topic,
        partition,
      ...
      val record3 = new ProducerRecord[Array[Byte], Array[Byte]](
        topic,
        null,
        "key".getBytes(StandardCharsets.UTF_8),
        "value".getBytes(StandardCharsets.UTF_8)
      ...
      assertEquals(
        ...
        "Should have offset " + (numRecords + 4)
      )
  ...
  def testSendCompressedMessageWithCreateTime( /*quorum: String*/ ): Unit = {
    val producer = createProducer(compressionType = "gzip", lingerMs = Int.MaxValue, deliveryTimeoutMs = Int.MaxValue)
  ...
  protected def sendAndVerifyTimestamp(
      producer: KafkaProducer[Array[Byte], Array[Byte]],
   ...
   *
   * Producer will attempt to send messages to the partition specified in each record, and should succeed as long as the
   * partition is included in the metadata.
   */
  // @ParameterizedTest(name = TestInfoUtils.TestWithParameterizedQuorumName)
  // @ValueSource(strings = Array("zk", "kraft"))
  @Test
