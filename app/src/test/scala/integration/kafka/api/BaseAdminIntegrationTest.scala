...
/**
 * Base integration test cases for [[Admin]]. Each test case added here will be executed in extending classes. Typically
 * we prefer to write basic Admin functionality test cases in [[kafka.api.PlaintextAdminIntegrationTest]] rather than
 * here to avoid unnecessary execution time to the build. However, if an admin API involves differing interactions with
...
//     waitUntilBrokerMetadataIsPropagated(brokers)
  ...
  @Test
  @Disabled("Require CreateTopicRequest V4")
