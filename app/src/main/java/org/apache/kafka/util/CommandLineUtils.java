...
public class CommandLineUtils {
  ...
  /**
   * Check and print help message if there is no options or `--help` option from command line, if
   * `--version` is specified on the command line print version information and exit.
   *
   * @param commandOpts Acceptable options for a command
  ...
  public static void checkRequiredArgs(
      OptionParser parser, OptionSet options, OptionSpec<?>... requiredList) {
  ...
  public static void checkInvalidArgs(
      OptionParser parser,
      OptionSet options,
    ...
    if (options.has(usedOption)) {
      for (OptionSpec<?> arg : invalidOptions) {
        if (options.has(arg)) {
          printUsageAndExit(
              parser,
              String.format("Option \"%s\" can't be used with option \"%s\"", usedOption, arg));
        }
      }
    }
  }
  ...
  public static void checkInvalidArgsSet(
      OptionParser parser,
      OptionSet options,
      ...
      for (OptionSpec<?> arg : invalidOptions) {
        if (options.has(arg)) {
          printUsageAndExit(
              parser,
              String.format(
                  "Option combination \"%s\" can't be used with option \"%s\"%s",
  ...
  /** Parse key-value pairs in the form key=value. Value may contain equals sign. */
          ...
          if (split.length == 1
            ...
            if (acceptMissingValue) {
              ...
              throw new IllegalArgumentException(
                  String.format("Missing value for key %s}", split[0]));
            }
          } else {
  ...
  /**
   * Merge the options into {@code props} for key {@code key}, with the following precedence, from
   * high to low: 1) if {@code spec} is specified on {@code options} explicitly, use the value; 2)
   * if {@code props} already has {@code key} set, keep it; 3) otherwise, use the default value of
   * {@code spec}. A {@code null} value means to remove {@code key} from the {@code props}.
   */
  public static <T> void maybeMergeOptions(
      Properties props, String key, OptionSet options, OptionSpec<T> spec) {
