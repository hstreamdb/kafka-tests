...
// Copy from metadata/src/main/java/org/apache/kafka/metadata/BrokerState.java
 ...
 * The broker state.
 *
 * <p>The numeric values used here are part of Kafka's public API. They appear in metrics, and are
 * also sent over the wire in some cases.
 *
 * <p>The expected state transitions are:
 *
...
public enum BrokerState {
  ...
  /**
   * The broker has caught up with cluster metadata, but has not yet been unfenced by the
   * controller.
   */
  RECOVERY((byte) 2),

  /**
   * The state the broker is in when it has registered at least once, and is accepting client
   * requests.
   */
  RUNNING((byte) 3),

  /** The state the broker is in when it is attempting to perform a controlled shutdown. */
