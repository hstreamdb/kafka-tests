...
/**
 * This is a trick to prevent the compiler from inferring the `Nothing` type in cases where it would be a bug to do so.
 ...
 * be inferred, which is not desirable. As defined above, we get a helpful compiler error asking the user to provide the
